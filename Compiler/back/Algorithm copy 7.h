...
static  gimple_array *gimple_arraytest ;
static	gimple_array start;
...
void set_gimple_array(gimple_array *table, gimple *used_stmt, tree a, tree target)
{
	...
	// debug_tree(a);
	fprintf(stderr,"set_gimple_arrayset_gimple_arrayset_gimple_array%x--------\n",&(table));
	//fprintf(stderr,"treeeeeeeeeeeeeeeeeeeFOR_EACH_IMM_USE_STMT%x--------\n",&(gimple_arraytest->stmt));
	if (table->stmt == NULL)
	{
...
bool analyze_type(tree tree_typed, tree trace_tree)
{
	...
	case FUNCTION_TYPE:
	{
		...
		if (args == NULL_TREE)
			...
			while (args != NULL_TREE && TREE_VALUE(args))
			{
				//!= void_type_node
				// tree_typed_tmp=TREE_VALUE(args);
				return analyze_type(TREE_VALUE(args), trace_tree);
				if (TREE_CHAIN(args) != NULL_TREE)
					fprintf(stderr, "and ");
				else
					fprintf(stderr, "... "); // this node is not void_type_node and the next node is NULL_TREE, thus variadic arguments
	...
	{
		// fprintf(stderr, "\n--------------------------------------------\n");
		// debug_tree(TREE_TYPE(tree_typed));
		// debug_tree(trace_tree);
		struct ptr_info_def *pi1;
		struct pt_solution *pt1;
		struct ptr_info_def *pi2;
		...
		//  analyze_type(tree_typed, TREE_TYPE(trace_tree));
		// debug_tree(TREE_TYPE(TREE_TYPE(trace_tree)));
		if (TREE_TYPE(tree_typed) == TREE_TYPE(TREE_TYPE(trace_tree)))
		{
			// fprintf(stderr, "\a");
			// fprintf(stderr, "\nfuckkkkkkddddddddddddddddddddddddddddddddddddddddddddddddkkkkker\n");
			return true;
...
void new_search_imm_use(gimple_array *used_stmt, tree target, tree target2)
{
	...
	FOR_EACH_IMM_USE_STMT(use_stmt, imm_iter, target)
	{
			...
			if( !strcmp(get_tree_code_name(TREE_CODE(target)), "addr_expr")) 	{
				 fprintf(stderr,"幹你娘-2-------\n");
				use_stmt= next_imm_use_stmt (&(imm_iter));
				if(use_stmt ==NULL)
				break;
				 }
				//  fprintf(stderr,"幹你娘-1-------\n");
	...
	fprintf(stderr,"treeeeeeeeeeeeeeeeeeeENNNNNNNNNNNNNNNNNNNNNND--------\n");
		...
		if (gimple_code(use_stmt) == GIMPLE_ASSIGN)
		{
			...
			if (gimple_assign_lhs(use_stmt) && TREE_CODE(gimple_assign_lhs(use_stmt)) == SSA_NAME && gimple_assign_single_p(use_stmt))
			{
				...
				set_gimple_array(used_stmt, use_stmt, gimple_assign_lhs(use_stmt),  target2);
				fprintf(stderr, "lhshslhslhslhslhslhslshlshlshlshlsh------------------\n");
				debug(use_stmt);
				debug_tree (gimple_assign_lhs(use_stmt));
				fprintf(stderr, "lhshslhslhslhslhslhslshlshlshlshlsh------------------\n");
					if( !strcmp(get_tree_code_name(TREE_CODE(gimple_assign_lhs(use_stmt))), "addr_expr")) 	{
				 fprintf(stderr,"幹你娘--------\n");
				break;
				 }else
		...
		else if (gimple_code(use_stmt) == GIMPLE_PHI)
		{
			...
			if (gimple_phi_result(use_stmt) && TREE_CODE(gimple_phi_result(use_stmt)) == SSA_NAME)
			{
				...
				if (!exist)
				{
					...
					set_gimple_array(used_stmt, use_stmt,  gimple_phi_result(use_stmt), target2);
					// fprintf(stderr, "-----------------hiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii------------------\n");
					debug_tree(gimple_phi_result(use_stmt));
			...
			if (gimple_call_fn(use_stmt) == NULL)
				...
				if (!strcmp(name, "free") || !strcmp(name, "xfree"))
				{
				 ...
				 if( !strcmp(get_tree_code_name(TREE_CODE(gimple_call_fn(use_stmt))), "addr_expr")) 	{
					  ...
					  set_gimple_array(used_stmt, use_stmt, 		fundecl, target2);
				 }
				 else
					set_gimple_array(used_stmt, use_stmt, 			gimple_call_arg(use_stmt, 0), target2);
...
void printfPointerConstraint(ptb *ptable,gimple_array *used_stmt)
{
	gimple *u_stmt;
	gimple_array start;
//	start.stmt = NULL;
...
int insert_always_inline(int tmp)
{
	cgraph_node *node;
	const char *name;
	bool always_inline;
			...
			for (gimple_stmt_iterator gsi = gsi_start_bb(bb); !gsi_end_p(gsi); gsi_next(&gsi))
			{
				...
				if (is_gimple_call(gc))
				{
						...
						!strcmp(name, "strdup"))
					{
						...
						//noinline
						//always_inline
						always_inline = (DECL_DISREGARD_INLINE_LIMITS(node->decl) && lookup_attribute("noinline", DECL_ATTRIBUTES(node->decl)));
						if (!always_inline && !MAIN_NAME_P(DECL_NAME(node->decl)))
						{
							DECL_ATTRIBUTES(node->decl) = tree_cons(get_identifier("noinline"), NULL, DECL_ATTRIBUTES(node->decl));
