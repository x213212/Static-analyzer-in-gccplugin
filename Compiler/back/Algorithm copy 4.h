...
void printfPointerConstraint(ptb *ptable)
{
	...
	FOR_EACH_TABLE(table_temp, t)
	{
		...
		{
			if( !strcmp(get_tree_code_name(TREE_CODE(table_temp->target)), "addr_expr")) 	{ 	//	debug_tree(table_temp->target);
				 break;	}
			if (ptr_derefs_may_alias_p(table_temp->target, user_tmp->aptr)) 				{
			debug(u_stmt);
...
void PointerConstraint(ptb *ptable, ptb *ftable)
{
	...
	start2.stmt = NULL;
	used_stmt = &start2;
			// fprintf(stderr, "noewnoewnoewnoewnoew2 -----------------------------------------------\n");
