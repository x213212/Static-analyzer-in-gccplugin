...
void trace_fucntion_relate_stmt(cgraph_node *node, tree function_tree, tree mallocStmt_tree, gimple *stmt)
{
							...
							for (t = DECL_ARGUMENTS(node->get_fun()->decl); t; t = DECL_CHAIN(t))
							{
								...
								if (gimple_assign_lhs(gc) != NULL)
								{
									...
									// if (ptr_derefs_may_alias_p(mallocStmt_tree, gimple_assign_lhs(gc)))
									{
										...
										warning_at(gimple_location(stmt), 0, "use location");
								...
								if ((TREE_CODE(gimple_assign_rhs1(gc)) == SSA_NAME))
								{
									...
									if (def_stmt != NULL)
										if ((gimple_code(def_stmt) != GIMPLE_PHI) && ptr_derefs_may_alias_p(mallocStmt_tree, gimple_assign_lhs(gc)) && ptr_derefs_may_alias_p(mallocStmt_tree, gimple_assign_rhs1(gc)))
										{
											...
											fprintf(stderr, "\033[40;36m ======= relate gimple_assig2n_lhs:%s========= \033[0m\n", gimple_assign_lhs(gc));
...
int trace_function_path(tree target_tree, int fucntion_level, tree mallocStmt_tree, int *freecount, gimple *gc, tree function_tree)
{
		...
		if (function_free_collect->get(function_tree) != NULL)
		{
			...
			for (int k = 0; k < callerRetTypearray.size(); k++)
			{
				if (ptr_derefs_may_alias_p((callerRetTypearray)[k].free_tree, mallocStmt_tree))
				{
					...
					++(*freecount);
					// fprintf(stderr, "qweeeeeeeeeeeeeeeeeeeeeeee%s\n", get_name(mallocStmt_tree));
	...
	if (tracerelatestmt == true)
	{
		...
		if (mallocStmt_tree != NULL)
		{
			...
			if (gc != NULL)
			{
				fprintf(stderr, "dot graph entry %s\n", (char *)get_name(target_tree));
				fprintf(stderr, "dot graph target entry end\n\n");
				trace_fucntion_relate_stmt(node, target_tree, mallocStmt_tree, gc);
			}
		}
	}
	if (fucntion_level != -1)
