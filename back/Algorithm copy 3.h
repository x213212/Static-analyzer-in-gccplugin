...
void FunctionStmtMappingRet(ptb *ptable, gimple_array *user_tmp){

void collect_FunctionMapping_Assign(tree function_tree);

void collect_FunctionMapping_Ret(tree function_tree,	gimple *u_stmt, gimple_array *user_tmp);
...
void collect_FunctionMapping_Assign(gimple *gc, cgraph_node *node, basic_block bb)
{
	...
	if (gimple_code(gc) == GIMPLE_CALL){
		...
		if (function_return_collect->get(test2) == NULL)
			return;
		function_return_array fun_array = *(function_return_collect->get(test2));
			...
			if (table_temp->target != NULL)
			{
				...
				if (user_tmp != NULL)
				{
					...
					if (user_tmp->size > 0)
						...
						{
							debug(u_stmt);
							collect_FunctionMapping_Ret(cfun->decl,u_stmt,user_tmp);
							// user_tmp->
						}
				}
				else
...
void PointerConstraint(ptb *ptable, ptb *ftable)
{
	...
	FunctionStmtMappingRet(ptable, used_stmt);
